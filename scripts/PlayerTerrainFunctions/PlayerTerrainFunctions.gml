...
function player_find_floor(height)
{
	...
	{
		if (player_beam_collision(solid_colliders, x_radius, oy) != noone)
		{
			return oy;
		}
	}
...
function player_find_ceiling(height)
{
	for (var oy = 0; oy <= height; ++oy)
	{
		var inst = player_beam_collision(solid_colliders, x_radius, -oy);
...
/* AUTHOR NOTE:
(1) Wall collisions are checked by calling `player_beam_collision(solid_colliders)`.
(2) If you simply wanted to check for the presence of a floor or ceiling, you can pass the `solid_colliders` array to the `player_part_collision` function alongside your desired height. */